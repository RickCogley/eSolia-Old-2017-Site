        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data.responseData.feed.entries, function (e, item) {
                    s += '<li><a class="green-text text-accent-3" href="' + item.link + '" target="' + def.TitleLinkTarget + '" >' + item.title + "</a>";
                    if (def.ShowPubDate){
                        ...
                        if ($.trim(def.DateFormat).length > 0) {
                            ...
                            s += '<div class="itemDate blue-grey-text text-darken-1">' + dt.toLocaleDateString() + "</div>";
                        }
                    }
                    if (def.ShowDesc) {
